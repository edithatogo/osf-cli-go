...
- [x] Task: Audit datalad/datalad-osf capabilities, activity, license, tests, release maturity, and user workflows
...
- [x] Task: Add failing offline tests for each accepted capability gap (no new accepted gap; existing API and file safety tests cover the shared surface)
- [x] Task: Implement accepted capabilities using existing repository patterns (no new API capability required; DataLad-specific protocols are deferred)
    - [x] Preserve conservative write and authentication behavior
    - [x] Add CLI and MCP exposure where applicable (shared OSF surface already exposed)
...
- [x] Task: Reconcile GitHub issue #12 with evidence and remaining external validation
