...
| Smithery | Published | MCPB bundle or public Streamable HTTP endpoint | `smithery mcp publish ./artifact.mcpb -n edithatogo/osf-cli-go` or URL publish | Release `4a285e7c-567f-4c53-ae1d-af64e95fc054` accepted |
| MCP.Directory | Prepared | Public GitHub repo and MCP registry entry | Manual submit form | Browser receipt text: `Server Submitted!`; external listing/review remains outside repo-local control |
...
| Codex plugin | Prepared | `plugins/codex-osf` and `.agents/plugins/marketplace.json` | Repo/local plugin install | `codex plugin marketplace add` accepted local marketplace |
