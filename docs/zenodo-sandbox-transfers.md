...
- Current bucket upload receipts use `version_id`, `key`, `size`, and checksum;
  the adapter then refreshes the deposition inventory to obtain its authoritative
  download URL. Inventory sizes may be integral JSON decimals such as `22.0`.
- Downloads use an atomic partial file, validate byte-range continuation,
  ...
  cancellation.
- Download requests use neutral HTTP content negotiation because the sandbox
  content endpoint rejects a forced `application/octet-stream` Accept header.
...
The 2026-07-15 run is live-validated with a one-use `deposit:write` token. The
token excluded publication and email scopes and was revoked after the evidence
