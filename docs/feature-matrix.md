...
| OSF metadata validation | validate --profile research-output|preregistration | node, contributor, and storage metadata | not exposed | read-only deterministic findings; no LLM or scientific validity claim | implemented | Maintain finding schema compatibility | `conductor/tracks/metacheck-osf-validation_20260711/spec.md` | #20 |
